...
void BackgroundSubtractorIMBS::deleteAll() {
	for(unsigned int p = 0; p < numPixels; ++p) {
		delete[] bgBins[p].binValues;
		delete[] bgBins[p].binHeights;
		delete[] bgBins[p].isFg;
	
		delete[] bgModel[p].values;
		delete[] bgModel[p].isValid;
		delete[] bgModel[p].isFg;
		delete[] bgModel[p].counter;
	}
...
void BackgroundSubtractorIMBS::apply(InputArray _frame, OutputArray _fgmask, bool &background_generated)
{
  ...
  /*
                  if(prev_prev_frame.data)
                          imshow("prev", prev_prev_frame);
                  imshow("curr", frame);
  */
                  ...
                  if(prev_frame.data) {
                          ...
                          prev_prev_fgmask = prev_fgmask.clone();
                  }
                  prev_frame = frame.clone();
                  prev_fgmask = fgmask.clone();
...
void BackgroundSubtractorIMBS::createBg(unsigned int bg_sample_number) {
	...
	if(bg_sample_number == (numSamples - 1)) {
		...
		//cout << "bgFilename " << bgFilename << endl;



			stringstream ss;
			ss << nframes;
			string bgImageName = "bg_" + ss.str() + ".png";
			if(imwrite(bgImageName, bgImage)) {
// 				cout << "file " << bgImageName << " saved." << endl;
			}
			else {
// 				cout << "unable to save file " << bgImageName << endl;
			}

		if(bgFilename.compare("") != 0) {
			string bgFileID = "bg_" + ss.str() + ".txt";


			ofstream file;
			//file.open(bgFilename->c_str());

			file.open(bgFileID.c_str());
			if(!file.is_open()) {
				cout << "Unable to open file " << bgFileID << endl;
				exit(EXIT_FAILURE);
			}
			else {
// 				cout << "Writing on file " << bgFileID << endl;
			}
...
void BackgroundSubtractorIMBS::opticalFlow() {
	...
	cv::cvtColor (frame, curr_frame_gray, CV_BGR2GRAY);
	cv::cvtColor (prev_prev_frame, prev_frame_gray, CV_BGR2GRAY);

	cv::Mat diffImage;

	cv::subtract(prev_frame_gray, curr_frame_gray, diffImage);
	...
	const int win_size = 15;
	 ...
	 cv::namedWindow ("prev OF", 0);

	 cv::namedWindow ("frame OF", 0);

   	cv::imshow ("prev OF", prev_frame);

	 cv::imshow ("frame OF", frame);
	*/
	    ...
	    cv::Mat dst_img = cv::Mat::zeros (frameSize, CV_8UC1);
	    ...
	    for (unsigned int i = 0; i < features_found.size (); i++)
	    {
	      ...
	      cv::line (dst_img, p0, p1, CV_RGB (255, 0, 255), 2);
	    }

	    cv::namedWindow ("Optical flow");
	    cv::imshow ("Optical flow", dst_img);
	...
	// create two images and allocate other structures
	Mat gray_frame;
	cvtColor(frame, gray_frame, CV_BGR2GRAY);
	Mat gray_mask;
	gray_frame.copyTo(gray_mask, fgmask);

	Mat gray_prev_frame;
	cvtColor(prev_frame, gray_prev_frame, CV_BGR2GRAY);
	Mat gray_prev_mask;
	gray_prev_frame.copyTo(gray_prev_mask, prev_fgmask);
...
/*
	...
	calcOpticalFlowFarneback(gray_prev_mask, gray_mask, flow, 0.5, 5, 5, 3, 7, 1.5, OPTFLOW_FARNEBACK_GAUSSIAN);
	cvtColor(gray_prev_mask, cflow, CV_GRAY2BGR);
	...
	for(int y = 0; y < cflow.rows; y += step) {
		for(int x = 0; x < cflow.cols; x += step)
		{
			...
			line(cflow, Point(x,y), Point(cvRound(x+fxy.x), cvRound(y+fxy.y)), color);
			//circle(cflow, Point(x,y), 2, color, -1);
		}
...
*/
	imshow("mask", gray_mask);
	imshow("prev mask", gray_prev_mask);
	waitKey(0);
	...
	goodFeaturesToTrack(gray_prev_mask, points[0], MAX_COUNT, 0.01, 10, Mat(), 3, 0, 0.04);
	cornerSubPix(gray_prev_mask, points[0], subPixWinSize, Size(-1,-1), termcrit);

	vector<uchar> status;
	vector<float> err;

	calcOpticalFlowPyrLK(gray_prev_mask, gray_frame, points[0], points[1], status, err, winSize,
	                                         3, termcrit, 0, 0.001);
	size_t i, k;
	for( i = k = 0; i < points[1].size(); i++ )
	{
			...
			circle( gray_frame, points[0][i], 3, Scalar(0,0,255), -1, 8);
			circle( gray_frame, points[1][i], 3, Scalar(0,255,0), -1, 8);
	}
	points[1].resize(k);

	imshow("LK Demo", gray_frame);
...
void BackgroundSubtractorIMBS::getBgModel(BgModel bgModel_copy[], unsigned int size) {
	...
	for (unsigned int p = 0; p < numPixels; ++p) {
		bgModel_copy[p].values = new Vec3b[maxBgBins];
		bgModel_copy[p].isValid = new bool[maxBgBins];
		bgModel_copy[p].isValid[0] = false;
		bgModel_copy[p].isFg = new bool[maxBgBins];
		bgModel_copy[p].counter = new uchar[maxBgBins];
...
void BackgroundSubtractorIMBS::saveBg(string& bgFilename) {
