...
public:
    ...
    ~BackgroundSubtractorIMBS();
    //! the update operator
    void apply(InputArray image, OutputArray fgmask, bool &background_generated);
    inline void resetBg() { deleteAll(); nframes = 0; }
...
private:
    ...
    Mat fgmask;
    //current filtered foreground mask
    Mat fgfiltered;

    //previous input RGB frame
    Mat prev_frame;
    //previous foreground mask
    Mat prev_fgmask;

    Mat prev_prev_frame;
    Mat prev_prev_fgmask;
