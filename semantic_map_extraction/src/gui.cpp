...
void GUI::drawRobot(const cv::Point2i cell_robot, const float theta_robot, const int img_size_x, const int img_size_y) {
	if(this->can_start != false) {
		...
		cv::Point robot_pt(cell_robot.x*ratio_x + ratio_x/2, cell_robot.y*ratio_y + ratio_y/2);
		cv::Point robot_oriented(cell_robot.x*ratio_x + ratio_x/2*(1 + cos(theta_robot)), cell_robot.y*ratio_y + ratio_y/2*(1 - sin(theta_robot)));
