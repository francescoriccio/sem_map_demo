#include "semantic_map_extraction.hpp"
...
void SemanticNode::check_object_area(std::vector<cv::Point2i> cell) {
	...
	try {
		listener->waitForTransform("/map", "/" + this->robotname + "/base_link", ros::Time(0), ros::Duration(3));
		listener->lookupTransform("/map", "/" + this->robotname + "/base_link", ros::Time(0), transform);
	}
	...
	x_robot = (this->resolution != -1) ? (x_robot - this->origin_x)/this->resolution : x_robot;
	y_robot = (this->resolution != -1) ? this->image.size().height - (y_robot - this->origin_y)/this->resolution : y_robot;
	cv::Point2i robot_cell = (this->grid_map)->getCellFromImageCoords(cv::Point2i(x_robot, y_robot));
			
	for(std::vector<cv::Point2i>::iterator i = cell.begin(); i != cell.end(); i++) {
		try{
			if(((this->semantic_matrix)->getCellByIndexes(i->y, i->x))->getRoomTag().empty()) {
				((this->semantic_matrix)->getCellByIndexes(i->y, i->x))->setRoomTag(((this->semantic_matrix)->getCellByIndexes(robot_cell.y, robot_cell.x))->getRoomTag());
				this->imposed_tags.push_back(std::make_pair(((this->semantic_matrix)->getCellByIndexes(robot_cell.y, robot_cell.x))->getRoomTag(), cv::Point(i->x, i->y)));
			}
		}
...
void SemanticNode::areasUpdate() {
	...
	for(std::vector<std::pair<std::string, cv::Point> >::const_iterator it = this->tags.begin(); it != this->tags.end(); it++)
		tag_coords.push_back((*it).second);

	(this->room_map)->extractRooms(tag_coords);
	
	try {
		(this->semantic_matrix)->setRoomTags(this->tags, *this->room_map, this->horizontal_cell_centers, this->vertical_cell_centers);
	}
	catch(char const* str) {
		ROS_ERROR("%s", str);
	}
	
	for(std::vector<std::pair<std::string, cv::Point> >::const_iterator it = this->imposed_tags.begin(); it != this->imposed_tags.end(); it++) {
		try {
			((this->semantic_matrix)->getCellByIndexes(it->second.y, it->second.x))->setRoomTag(it->first);
		}
		catch(char const* str) {
			ROS_ERROR("%s", str);
		}
...
void SemanticNode::callService() {
 	ros::ServiceClient client = n->serviceClient<semantic_map_extraction::GenerateKB>("/create_topological_graph");
	...
	if(this->resolution != -1) {
		for(std::vector<float>::iterator i = this->horizontal_cell_centers.begin(); i != this->horizontal_cell_centers.end(); i++)
			real_world_cell_centers_x.push_back((*i) * this->resolution + this->origin_x);
			
		for(std::vector<float>::iterator i = this->vertical_cell_centers.begin(); i != this->vertical_cell_centers.end(); i++)
			real_world_cell_centers_y.push_back(-(*i - this->image.size().height) * this->resolution + this->origin_y);
	}
	else {
	...
	std::cout << "Matrix Width: " << this->horizontal_distances.size() << ", height " << this->vertical_distances.size()
						<< ", total cells " << this->horizontal_distances.size()*this->vertical_distances.size() << std::endl;
	
	std::cout << "Image width: " << this->image.cols << ", image height " << this->image.rows << ", total pixels " << this->image.cols*this->image.rows << std::endl;
	...
	if(this->wait_service)
		ros::service::waitForService("/create_topological_graph");

	if(client.call(msg))
		ROS_INFO("Semantic matrix sent");
	else
		ROS_INFO("Failed to call service CreateTopologicalGraph");
}
...
void SemanticNode::loadXMLMap(std::string XMLFilePath) {
	...
	if (xml.LoadFile()) {
		...
		for(; elem; elem = elem->NextSiblingElement()) {
			...
			if(tag)
				this->imposed_tags.push_back(std::make_pair(std::string(tag), cv::Point(x, y)));
		}
		...
		for(; elem; elem = elem->NextSiblingElement()) {
			...
			if(obj && properties) {
				this->objects.push_back(Objects(obj, cv::Point2i(x, y), theta, cv::Vec3f(dimX, dimY, dimZ), properties, (type) object_type));
...
void SemanticNode::saveXMLMap(std::string XMLFilePath) {  
	...
	comment->SetValue("Semantic map saved XML file. Coords are already image coordinates.");
	...
	root->LinkEndChild(areas);
 	...
 	int tidx = 0;
	for(std::vector<std::pair<std::string, cv::Point> >::iterator i = this->tags.begin(); i != this->tags.end(); i++) {
		if((this->static_tags[tidx] && XMLFilePath.compare(this->StatXMLFilePath) == 0) || (!this->static_tags[tidx] && XMLFilePath.compare(this->DynXMLFilePath) == 0)) {
			TiXmlElement* tag;
	...
	for(std::vector<std::pair<std::string, cv::Point> >::iterator i = this->imposed_tags.begin(); i != this->imposed_tags.end(); i++) {
		TiXmlElement* tag;
		cv::Point pt = i->second;
	...
	root->LinkEndChild(objs);
 	...
 	int idx = 0;
	for(std::vector<Objects>::iterator i = this->objects.begin(); i != this->objects.end(); i++) {
		if((this->static_objects[idx] && XMLFilePath.compare(this->StatXMLFilePath) == 0) || (!this->static_objects[idx] && XMLFilePath.compare(this->DynXMLFilePath) == 0)) {
			TiXmlElement* obj;
...
SemanticNode::SemanticNode(ros::NodeHandle *n, std::string robotname, std::string path, std::string StatXMLFilePath, std::string DynXMLFilePath, int timeout, bool wait_service, bool load_dyn_map) {
	this->n = n;
...
SemanticNode::SemanticNode(ros::NodeHandle *n, const cv::Mat &image, std::string robotname, std::string path, std::string StatXMLFilePath, std::string DynXMLFilePath, int timeout, bool wait_service, bool load_dyn_map) {
	this->n = n;
	this->robotname = robotname;
...
void SemanticNode::addElement() {
	...
	Objects ob(oss.str(),this->buffer.pt,this->buffer.angle,this->buffer.size,this->buffer.properties,this->buffer.ob_type);
...
void SemanticNode::mapProcessing() {
	...
	for(std::vector<float>::iterator i = this->horizontal_distances.begin(); i != this->horizontal_distances.end(); i++) {
	...
	for(std::vector<float>::iterator i = this->vertical_distances.begin(); i != this->vertical_distances.end(); i++) {
	...
	semantic_matrix = new SemanticMatrix(this->vertical_distances.size(), this->horizontal_distances.size());
}
...
void SemanticNode::ObservationTopicSubscriber(const semantic_map_extraction::Obs::ConstPtr& observation) {
	float object_x = (this->resolution != -1) ? (observation->posx - this->origin_x)/this->resolution : observation->posx;
	float object_y = (this->resolution != -1) ? this->image.size().height - (observation->posy - this->origin_y)/this->resolution : observation->posy;
	float dim_x = (this->resolution != -1) ? observation->dimx/this->resolution : observation->dimx;
	float dim_y = (this->resolution != -1) ? observation->dimy/this->resolution : observation->dimy;
	float dim_z = (this->resolution != -1) ? observation->dimz/this->resolution : observation->dimz;
...
//	// DEBUG // 
//	std::cerr << "Received POSX=" << observation->posx << ", POSY=" << observation->posy << ", THETA=";
//	std::cerr << observation->theta << ", DIMS (X,Y,Z) = (" << observation->dimx << ", " << observation->dimy << ", " << observation->dimz << ") ";
//	std::cerr << "Converted image coords POSX=" << object_x << ", POSY=" << object_y << ", THETA=";
//	std::cerr << observation->theta << ", DIMS (X,Y,Z) = (" << dim_x << ", " << dim_y << ", " << dim_z << ") " << std::endl;
//	std::cerr << "PARAMS = (" << observation->properties << ") " << std::endl;
	...
	if(this->buffer.time != ros::Time(0) && (ros::Time::now() - this->buffer.time).toSec() < this->timeout && buffer.last_writer.compare("obs") != 0) {
		this->addElement();
		this->buffer.time = ros::Time(0);
...
void SemanticNode::ASRSubscriber(const std_msgs::String::ConstPtr& string) {
	...
	try {
		...
		this->buffer.ob_type = (this->buffer.name.find("door") != std::string::npos || this->buffer.name.find("DOOR") != std::string::npos) ? DOOR : NORMAL;

		if(this->buffer.time != ros::Time(0) && (ros::Time::now() - this->buffer.time).toSec() < this->timeout && buffer.last_writer.compare("asr") != 0) {
                        this->addElement();
                        this->buffer.time = ros::Time(0);
...
void SemanticNode::RemoveObjectSubscriber(const std_msgs::String::ConstPtr& string) {
...
void SemanticNode::RobotPoseTimer(const ros::TimerEvent&) {
	...
	if(this->resolution != -1) {
		try {
			this->listener->waitForTransform("/" + this->robotname + "/map", "/" + this->robotname + "/base_link", ros::Time(0), ros::Duration(1.0));
			this->listener->lookupTransform("/map", "/" + this->robotname + "/base_link", ros::Time(0), transform);
		}
		catch(tf::TransformException ex) {
			ROS_ERROR("%s", ex.what());
		}
		...
		y_robot = this->image.size().height - (transform.getOrigin().y() - this->origin_y)/this->resolution;
		theta_robot = tf::getYaw(transform.getRotation());
	}
	
	if(grid_map != 0) {
		cv::Point2i cell(grid_map->getCellFromImageCoords(cv::Point2i(x_robot, y_robot)));
		(this->sem_gui)->drawRobot(cell, theta_robot, horizontal_distances.size(), vertical_distances.size());
	}
}


bool SemanticNode::cellByCoordsHandler(semantic_map_extraction::GetCellByCoords::Request &req, semantic_map_extraction::GetCellByCoords::Response &res) {
	float coord_x, coord_y;

	coord_x = (this->resolution != -1) ? (req.real_x - this->origin_x)/this->resolution : req.real_x;
	coord_y = (this->resolution != -1) ? this->image.size().height - (req.real_y - this->origin_y)/this->resolution : req.real_y;

	cv::Point2i cell(grid_map->getCellFromImageCoords(cv::Point2i(coord_x, coord_y)));
...
bool SemanticNode::coordsByCellHandler(semantic_map_extraction::GetCoordsByCell::Request &req, semantic_map_extraction::GetCoordsByCell::Response &res) {
	cv::Point2i coords(grid_map->getImageCoordsFromCell(cv::Point2i(req.cell_x, req.cell_y)));
	res.real_x = (this->resolution != -1) ? coords.x*this->resolution + this->origin_x : coords.x;
	res.real_y = (this->resolution != -1) ? -(coords.y - this->image.size().height)*this->resolution + this->origin_y : coords.y;

	return true;
}


bool SemanticNode::deleteMapHandler(semantic_map_extraction::DeleteMap::Request &req, semantic_map_extraction::DeleteMap::Response &res) {
...
void SemanticNode::showImages() {
	...
	try {
		...
		(this->semantic_matrix)->showMapImage(this->image, horizontal_distances, vertical_distances);
	}
	catch(char const* str) {
		ROS_ERROR("%s", str);
	}
...
void SemanticNode::saveImages(std::string basename) {
	...
	try {
		std::string image_tmp = (this->semantic_matrix)->saveMatrixImage(this->path, basename);
		(this->semantic_matrix)->saveMapImage(this->image, horizontal_distances, vertical_distances, this->path, basename);
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "semantic_map_extraction");
	ros::NodeHandle n("~");
	std::string path;
	...
	n.param<std::string>("/robotname", robotname, "robot0");
	n.param<std::string>("statXMLFilePath", StatXMLFilePath, "./mapXMLfile.xml");
	n.param<std::string>("dynXMLFilePath", DynXMLFilePath, "./augmentedMapXMLfile.xml");
	...
	error_msg << "Usage: semantic_map_extraction [MODALITY] [FILE]" << std::endl << "MODALITY: 0 if you want to use the map_server, 1 otherwhise" << std::endl
	...
	if(modality == 0) {
		sem_node = new SemanticNode(&n, robotname, path, StatXMLFilePath, DynXMLFilePath, timeout, wait_service, load_dyn_map);

		sub_map = n.subscribe<nav_msgs::OccupancyGrid>("/map", 1000, &SemanticNode::mapSubscriber, sem_node);
	}
	else {
		...
		sem_node = new SemanticNode(&n, src, robotname, path, StatXMLFilePath, DynXMLFilePath, timeout, wait_service, load_dyn_map);
	...
	sub_obs = n.subscribe<semantic_map_extraction::Obs>("/" + robotname + "/ObservationTopic", 1000, &SemanticNode::ObservationTopicSubscriber, sem_node);
	sub_asr = n.subscribe<std_msgs::String>("/" + robotname + "/ASR", 1000, &SemanticNode::ASRSubscriber, sem_node);
	sub_remove_object_by_name = n.subscribe<std_msgs::String>("/remove_object_by_name", 1000, &SemanticNode::RemoveObjectSubscriber, sem_node);
	srv_cell_by_coords = n.advertiseService("get_cell_by_coords", &SemanticNode::cellByCoordsHandler, sem_node);
	srv_coords_by_cell = n.advertiseService("get_coords_by_cell", &SemanticNode::coordsByCellHandler, sem_node);
	srv_delete_map = n.advertiseService("delete_map", &SemanticNode::deleteMapHandler, sem_node);
	
	robot_pose_timer = n.createTimer(ros::Duration(0.1), &SemanticNode::RobotPoseTimer, sem_node);
