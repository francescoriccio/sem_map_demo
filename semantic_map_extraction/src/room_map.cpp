...
void RoomMap::addDoors(std::vector<std::pair<cv::Point2i,cv::Point2i> > doors) {
	if(doors.size() == 0) {
		cv::Mat tmp;
	...
	for(std::vector<std::pair<cv::Point2i,cv::Point2i> >::const_iterator i = doors.begin(); i != doors.end(); i++)
		cv::line(this->original_with_doors_map, i->first, i->second, cv::Scalar(0), 0.5, CV_AA);
}

void RoomMap::addDoors(const std::pair<cv::Point2i,cv::Point2i> &door) {
	cv::line(this->original_with_doors_map, door.first, door.second, cv::Scalar(0), 0.5, CV_AA);
}

void RoomMap::extractRooms(const std::vector<cv::Point> &tags) {
	cv::Mat	thresholded(this->original_map.size(), CV_8U), markers(this->original_map.size(), CV_32S);
	int compCount = 200;
	...
	cv::threshold(this->original_map, thresholded, 220, 255, cv::THRESH_BINARY_INV);
	cv::circle(markers, cv::Point(1,1), 1, cv::Scalar(compCount), -1);

	for(std::vector<cv::Point>::const_iterator i = tags.begin(); i != tags.end(); i++, compCount++)
	...
	for(int i = 0; i < compCount; i++)
		colorTab.push_back(cv::Vec3b((uchar)cv::theRNG().uniform(0, 255), (uchar)cv::theRNG().uniform(0, 255), (uchar)cv::theRNG().uniform(0, 255)));
