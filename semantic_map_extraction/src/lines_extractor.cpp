...
bool LinesExtractor::isSameAngle(const float angle1, const float angle2) {
	return (std::fabs(std::fmod(angle1, (float) CV_PI) - std::fmod(angle2, (float) CV_PI)) < .01f);
}
...
void LinesExtractor::extract() {
	...
	for(int j = this->max_threshold; j > this->min_threshold; j--) {
		...
		float min_distance = std::max(this->max_threshold/(float)j, this->min_line_distance);
		...
		for(std::vector<cv::Vec2f>::iterator j = i + 1; j != this->extracted_lines.end(); j++) {
			if(this->isSameAngle((*i)[1], (*j)[1])) {
				if(this->isSameAngle((*i)[1], 0.0f))
					this->max_grid_spacing.width = std::min(fabs((*i)[0] - (*j)[0]), (double) this->max_grid_spacing.width);
				else
					this->max_grid_spacing.height = std::min(fabs((*i)[0] - (*j)[0]), (double) this->max_grid_spacing.height);
			}
		}
	}
}
...
bool LinesExtractor::checkActivePixel(int row, int col, cv::Point2i &good_pt, cv::Point2i &good_pt_inv) {
	int range_pixels = 0;
	cv::Point2i pt1(std::max(0, col - this->active_pixel_range), std::max(0, row - this->active_pixel_range));
	cv::Point2i pt2(std::min(this->image.cols - 1, col + this->active_pixel_range), std::min(this->image.rows - 1, row + this->active_pixel_range));
	...
	if(float(total_points)/float(this->active_pixel_range*this->active_pixel_range) > 0.5f)
		return false;
	...
	for(size_t i = 0; i < total_points; i++) {
		...
		for(size_t j = std::min(i + 1, total_points); j < total_points; j++) {
			...
			if(pt == -pt_inv) {
				...
				if(range_pixels == 0 || std::abs(pt.x - pt_inv.x) < std::abs(good_pt.x - good_pt_inv.x) || std::abs(pt.y - pt_inv.y) < std::abs(good_pt.y - good_pt_inv.y)) {
	...
	if(range_pixels > this->active_pixel_threshold && float(range_pixels)/float(total_points) < 0.08 && range_pixels < 0.35*this->active_pixel_range)
		return true;
			
	return false;
}

std::vector<std::pair<cv::Point2i, cv::Point2i> > LinesExtractor::computeDoors() {
	for(int row = 0; row < this->image.rows; row+=2) {
		for(int col = 0; col < this->image.cols; col+=2) {
			...
			if(this->checkActivePixel(row, col, pt, pt_inv)) {
				...
				for(i = this->doors.begin(); i != this->doors.end() && cv::norm(cv::Point2i((i->first.x + i->second.x)/2, (i->first.y + i->second.y)/2)
																																				 - cv::Point2i((pt.x + pt_inv.x)/2, (pt.y + pt_inv.y)/2)) > 2*this->active_pixel_range; ++i);
				...
				if(i == this->doors.end())
					...
					bool check_x = new_pt_xDist > 0.6*norm && old_pt_xDist > 0.6*old_norm && new_pt_xDist < old_pt_xDist;
					bool check_y = new_pt_yDist > 0.6*norm && old_pt_yDist > 0.6*old_norm && new_pt_yDist < old_pt_yDist;
...
std::vector<float> LinesExtractor::getLinesByAngle(const std::vector<cv::Vec2f> &lines, const float angle) {
	std::vector<float> good_angle;

	for(std::vector<cv::Vec2f>::const_iterator i = lines.begin(); i != lines.end(); i++)
...
void LinesExtractor::printLinesOnImage(cv::Mat &draw, const std::vector<cv::Vec2f> &lines) {
...
void LinesExtractor::printLinesFromPointsOnImage(cv::Mat &draw, const std::vector<std::pair<cv::Point2i, cv::Point2i> > &points) {
	cv::cvtColor(draw, draw, CV_GRAY2BGR);

	for(std::vector<std::pair<cv::Point2i, cv::Point2i> >::const_iterator i = points.begin(); i != points.end(); i++) {
		cv::line(draw, i->first, i->second, cv::Scalar(0,0,255), 0.5, CV_AA);
	}
}

LinesExtractor::LinesExtractor(const cv::Mat &image, const cv::Mat &original_image, const int max_threshold, const int min_threshold, const float min_line_distance, const int active_pixel_range, const int active_pixel_threshold) {
...
cv::Mat LinesExtractor::getExtractedImage() {
	...
	cv::threshold(extracted_image, extracted_image, 100, 255, cv::THRESH_BINARY_INV);
...
cv::Mat LinesExtractor::getActivePixelImage() {
	...
	cv::threshold(active_pixels_image, active_pixels_image, 100, 255, cv::THRESH_BINARY_INV);
