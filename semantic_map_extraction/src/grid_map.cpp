...
std::vector<float> GridMap::computeLineDistances(std::vector<float> lines) {
	...
	for(std::vector<float>::iterator i = lines.begin(); i != lines.end() - 1; i++)
...
void GridMap::showStack() {
	...
	for(std::vector<cv::Mat>::iterator i = this->stack.begin(); i != this->stack.end(); i++, j++) {
...
cv::Point2i GridMap::getCellFromImageCoords(cv::Point2i coords) {
	...
	if(coords.x < dims.width && coords.y < dims.height && coords.x >= 0 && coords.y >= 0) {
		for(std::vector<float>::iterator i = this->horizontal_lines.begin(); coords.y >= *i; i++, cell.y++);
		for(std::vector<float>::iterator i = this->vertical_lines.begin(); coords.x >= *i; i++, cell.x++);
	}
...
cv::Point2i GridMap::getImageCoordsFromCell(cv::Point2i cell) {
	...
	if(cell.x <= (int) horizontal_distances.size() && cell.y <= (int) vertical_distances.size() && cell.x >= 0 && cell.y >= 0) {
		for(std::vector<float>::iterator i = vertical_distances.begin(); i <= vertical_distances.begin() + cell.y; coords.y += (i == vertical_distances.begin()) ? (*i)/2 : *i, i++);
		for(std::vector<float>::iterator i = horizontal_distances.begin(); i <= horizontal_distances.begin() + cell.x; coords.x += (i == horizontal_distances.begin()) ? (*i)/2 : *i, i++);
	}
