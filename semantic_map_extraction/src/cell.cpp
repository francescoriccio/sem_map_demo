...
std::string Cell::getVectorString(std::vector<std::string> str_vec) {
	...
	for(std::vector<std::string>::iterator i = str_vec.begin() ; i != str_vec.end(); i++)
		if(i == str_vec.begin())
			str = *i;
		else
			str += "#" + *i;
...
Cell::Cell(const std::string &room_tag, const std::vector<std::string> &objects, const std::vector<bool> &door_adjacency, const std::vector<std::string> &additionals) {
...
std::string Cell::toString() {
	std::string str;
	std::string area = (this->room_tag.empty()) ? "0" : this->room_tag;	

	if(this->room_tag.empty() && this->objects.empty())
		str = "0";
	else
		str = area + ";" + getVectorString(this->objects) + ";" + getVectorString(this->door_adjacency) + ";" + getVectorString(this->additionals);
