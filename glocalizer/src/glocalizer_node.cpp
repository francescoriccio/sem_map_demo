...
class GLocalizerNode
{
  ...
  double init_cov_[3];
  bool first_map_received_, first_laser_received_;
...
main(int argc, char** argv)
{
  ...
  // Check if laser has been received!!!
  // first_laser_received_
        ...
        first_laser_received_(false)
{
  ...
  global_loc_srv_ = nh_.advertiseService("global_localization", 
					 &GLocalizerNode::globalLocalizationCallback,
                                         this);
  distances_pub_ = nh_.advertise<glocalizer::LocalizerRanges>("localizer_ranges",10);
...
GLocalizerNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // cout << "glocalizer:: Localized ? " << isLocalized << endl;
  if(isLocalized) {
