...
void doTurn(std::string robot_name, ros::Publisher &cmd_vel_pub, double target_angle_deg, 
            std::string absolute_relative_flag, double max_ang_vel_deg)
{
    ...
    while (!getRobotPose(robot_name,x,y,th_rad)) {
    ...
    while (fabs(adist)>min_diff_angle) {
        cmd.angular.z = (Kp*adist/fabs(Kp*adist))*std::min(std::fabs(Kp*adist), max_ang_vel);
